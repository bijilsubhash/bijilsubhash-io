...
Let's break that down by looking at what goes inside a SDP, shown below where we defining two tables: a streaming table (`raw_sales`) and a materialized view (`weekly_sales`)
...
A pipeline is the unit of development and execution, and it can contain one or more flows. It is a collection of source code files and configuration. The source files (`*.py` or `*.sql`) declare datasets along with the queries and flows that produce them. The execution of a pipeline is managed by a configuration file (pipeline.yml - see below) that specifies how the pipeline runs and where data is stored. With these artefacts in place, the pipeline can infer the dependencies and facilitate the execution of the respective flows as originally intended.

Here is a minimal `pipeline.yml` for the two datasets above. It points the pipeline at the source files, sets where streaming checkpoints live, and names the target catalog and database:
...
flowchart TD
    CLI["SDP CLI<br/>scaffold, validate, run"] --> PIPE
    subgraph PIPE[Pipeline]
        direction TB
        CFG[("Pipeline<br/>config")] -.-> SRC
        SRC["Source files<br/>(.py / .sql)"] --> RS
        subgraph DS[Datasets]
            direction TB
            RS["Streaming table<br/>raw_sales"]
            WS["Materialized view<br/>weekly_sales"]
        end
        RS -->|flow| WS
    end
    PIPE --> OUT[("Tables in<br/>catalog")]
```
