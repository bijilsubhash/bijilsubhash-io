...
![Imperative specifies the steps; declarative specifies the outcome](/img/imperative-vs-declarative.png)
...
Since the early Delta Live Tables (DLT) days, the team at Databricks, also Apache Spark's founding team, has been a great advocate for pushing Spark to be more declarative. Though it was generally well received, the proprietary, closed model of DLT meant it was limited to Databricks customers. The learnings and feedback from DLT, however, became the fuel for incorporating those principles into Apache Spark as SDP, improving its general accessibility while also facilitating the rebranding of DLT in Databricks as the new Lakeflow Declarative Pipelines (LDP), which under the hood uses SDP as its primary framework of execution.
...
Let's break that down by looking at what goes inside a SDP using an example where we defining two tables, a streaming table (`raw_sales`) and a materialized view (`weekly_sales`):
...
- **View**: a virtual table that does not persist results but instead saves a query definition, run against the source table when queried.

</Callout>

If you prefer SQL, here is the same in SQL:
...
None of the above looks new. PySpark and Spark SQL can produce both of those tables today, so on the face of it there's little reason to reach for another framework. The difference isn't in what the code does, it's in what the code no longer has to say.
...
To make the contrast concrete, the two decorators above unfold into a Structured Streaming job and a batch job that you own outright, checkpoints, triggers, and write semantics included:

```python
from pyspark.sql.functions import col, date_trunc, sum

# raw_sales - a streaming job, checkpoint and trigger and all
(
    spark.readStream.table("sales.raw_sales")
    .writeStream
    .option("checkpointLocation", f"{checkpoint_root}/raw_sales")
    .trigger(availableNow=True)
    .toTable("raw_sales")
)

# weekly_sales - a batch job, and you decide overwrite vs append
(
    spark.read.table("raw_sales")
    .withColumn("week_start", date_trunc("week", col("date")))
    .groupBy("week_start", "store_id")
    ...
    .saveAsTable("weekly_sales")
)
```

And that is only the compute. The order, the retries, the schedule, and the monitoring still sit around it, usually in a separate orchestrator.
...
A dataset is the object that lands in the database, essentially the physical layer. It can be a streaming table, materialized view, or view. The dataset is defined using a query, either in Python or SQL.

### Flows

A flow defines the semantics of how data is processed into a target dataset. It is generally auto-created based on how you define the dataset, but you do have the option to explicitly define a flow if needed for complex workflows. Depending on the target dataset, the flow determines whether the data is processed incrementally as a stream or via a full refresh of a physical table.
...
Here is a minimal configuration file for the two datasets above. It points the pipeline at the source files, sets where streaming checkpoints live, and names the target catalog and database:
...
storage: file:///pipelines/sales/storage
catalog: sales
database: analytics
configuration:
...
To see it work, here is that exact pipeline running locally end to end, from `spark-pipelines run` through to querying the resulting tables. The full setup is on GitHub as [sdp-sandbox](https://github.com/bijilsubhash/sdp-sandbox) if you want to run it yourself.

<Video
  src="/video/sdp-end-to-end.mp4"
  poster="/video/sdp-end-to-end.jpg"
  caption="Running a Spark Declarative Pipeline locally, end to end."
/>
...
Yes, but not easily, and there are some known gaps[^1]. This mainly boils down to the fact that it is still early days (it's only been ~6 months since GA). One that stood out to me is that a pipeline defined by SDP today is limited to a single trigger mode (`AvailableNow`) for the whole pipeline. Because that trigger drains the available data and then stops, you are effectively forced to run your streaming tables on a batch cadence. The processing is still incremental and checkpointed, but a streaming table only advances when the pipeline is triggered, and there is no continuous, always-on mode the way there is in LDP, so its freshness is capped by how often the whole pipeline runs. There are also some constraints on what knobs are available for managing retries in SDP. Lastly, SDP does not yet have the more efficient materialized view refresh you get in LDP. I see all of these as teething problems, and in fact some are already on the roadmap for future releases[^2].

[^1]: [Spark Declarative Pipelines, going further](https://www.waitingforcode.com/apache-spark-structured-streaming/spark-declarative-pipelines-going-further/read)
[^2]: [Spark Declarative Pipelines: Why Data Engineering Needs to Become End-to-End Declarative](https://www.databricks.com/blog/spark-declarative-pipelines-why-data-engineering-needs-become-end-end-declarative)
...
What Databricks has effectively done is hand the community the substrate of its own commercial product and bet on being better at productising it than anyone else, which is a remarkbly confident move and also the reason SDP is worth watching even if you don't adopt it this year.
