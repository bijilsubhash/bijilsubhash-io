...
```mermaid caption="A pipeline resolves a graph of datasets, each produced by a flow that reads a source and writes a target"
flowchart LR
    SRC[("Source<br/>sales.raw_sales")]
    subgraph P [Pipeline]
        direction LR
